...
def show_wofi(windows):
    command = 'wofi -a -W 600 -L ' + \
        str(len(windows) + 1) + \
        ' -o DP-2 -I -d -i -b -k /dev/null 2>/dev/null -c ~/.config/wofi/config -s ~/.config/wofi/style.css'
...
def get_windows():
    ...
    for con in cons:
        ...
        windows.append("img:id=" + str(con.get('id')) +
                       ":img:" + app_icon + ":text:" + name)
    return windows
